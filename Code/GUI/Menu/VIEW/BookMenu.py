...
class BookManaFrame(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        tk.Entry(search_frame, textvariable=self.search_var, width=40, font=self.pixel_font).pack(side='left',

                                                                                                  expand=True)
        tk.Button(search_frame, text='Search', command=self.search_book_in_data, font=self.pixel_font).pack(side='left',
                                                                                                    padx=5)
        ...
        button_load = macButton(button_frame, text="LOAD FORM", command=self.clear_form, font=self.pixel_font,
    ...
    def on_item_select(self, event):
        ...
        self.entry_isbn.config(state="readonly")
