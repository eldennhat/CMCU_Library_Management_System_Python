...
def search_or_load_all_copies():
    ...
    try:
        ...
        cursor = conn.cursor()
        # TODO: Sửa câu lệnh SQL để tìm kiếm (nếu cần)
        # query_text = entry_search.get()
        # if query_text:
        #     sql = "SELECT CopyId, BookId, PublisherName, Status, Barcode, BookMoney FROM BookCopy WHERE PublisherName LIKE %s"
